...
import sys
sys.stdout = open("Small/output"+time+".txt", "w")
...
smallOpt = {
    ...
    'batch_size': [1.,2, 3], 
    'etadrop': [0.75, .5, 0.25], # when dropping eta, multiply it by this number (e.g., .5 means halve it)
    'eta_frac': [0.3, 0.2, 0.1],  # drop eta every eta_frac fraction of the max iterations
    'lambda_' : [0.0025, 0.005, 0.0075]                   # so if eta_frac is .2, and maxiter is 10000, drop eta every 2000 iterations
}
