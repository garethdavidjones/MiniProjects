...
bigOpt = {
    'eta': [ 1.5, 1.0, 1.25, .85, .75, 0.6],   # initial learning rate
    'maxiter': [15000, 12000, 10000, 8000],  # max number of iterations (updates) of SGD
    'batch_size': [32, 34, 28, 30, 5, 10, ], 
    'etadrop': [.85, 0.8, 0.75, .70, 0.65], # when dropping eta, multiply it by this number (e.g., .5 means halve it)
    'eta_frac': [0.18, .2, 0.22],  # drop eta every eta_frac fraction of the max iterations
    'lambda_' : [0.015, 0.01, 0.05, 0.001, 0.0001, 0.00001]                  # so if eta_frac is .2, and maxiter is 10000, drop eta every 2000 iterations
}

print(bigOpt)

    
gs = GridSearchCV(softmaxModel(), bigOpt, cv=10, n_jobs=10)
...
# Save results
save_submission("submission-large.csv",  gs.predict(kaggleX))
